  ...
  <main class="journey-page">
    ...
    <el-tabs v-model="tab" class="journey-tabs" @tab-change="syncTab">
      ...
      <el-tab-pane label="急诊与呼救" name="emergency">
        <section class="panel-grid two"><el-card class="emergency-card"><template #header><b>一键紧急呼救</b></template><p>将定位、病史、过敏史和当前用药推送给已绑定家属与医生。</p><el-input v-model="emergency.locationText" placeholder="位置说明"/><el-button type="danger" size="large" :loading="busy" :disabled="!currentPatientId" @click="sos">发送紧急呼救</el-button></el-card><el-card><template #header><div class="card-head"><b>离线关键医疗信息卡</b><el-button @click="loadEmergencyCard">刷新</el-button></div></template><div v-if="emergencyCard"><h3>{{emergencyCard.patient?.name}}</h3><p><b>病史：</b> {{emergencyCard.patient?.medical_history||'-'}}</p><p><b>过敏史：</b> {{emergencyCard.clinical?.allergy_drugs||'-'}}</p><p><b>血型：</b> {{emergencyCard.clinical?.blood_type||'未记录'}}</p><p><b>当前用药：</b> {{(emergencyCard.medications||[]).map(x=>x.drug_name).join(', ')||'-'}}</p><el-tag>已缓存在本设备，可离线查看</el-tag></div></el-card></section>
      </el-tab-pane>
...
const clinicians=ref([]),doctorSchedules=ref([]),measurements=ref([]),appointments=ref([]),visits=ref([]),prescriptions=ref([]),specialtyRows=reactive({growth:[],vaccination:[],maternity:[]}),plans=ref([]),rehabRows=ref([]),emergencyCard=ref(null),mentalRows=ref([]),grants=ref([]),groups=ref([]),operations=ref(null)
...
async function sos(){if(busy.value||!currentPatientId.value)return;const selected=pid();busy.value=true;try{await ElMessageBox.confirm('这会向绑定的照护联系人发送您的位置和紧急医疗信息，是否继续？','紧急呼救',{confirmButtonText:'立即发送',type:'warning'})}catch{busy.value=false;return}if(currentPatientId.value!==selected){busy.value=false;return}try{if(navigator.geolocation)await new Promise(resolve=>navigator.geolocation.getCurrentPosition(p=>{emergency.latitude=p.coords.latitude;emergency.longitude=p.coords.longitude;resolve()},()=>resolve(),{timeout:5000}));if(currentPatientId.value!==selected)return;await api.triggerEmergency({...emergency,patientId:selected});ElMessage.success('紧急通知已发送')}catch{}finally{busy.value=false}}
...
async function loadMental(){await loadPatientData('mental',id=>api.listMentalAssessments(id),response=>{mentalRows.value=response.data||[]})}
...
async function scheduleMental(){await safely(async()=>{validate(mentalSchedule.nextDueAt && Number(mentalSchedule.intervalDays)>0,'请填写下次推送时间和有效间隔天数。');await api.saveMentalSchedule({...mentalSchedule,scaleCode:mental.scaleCode,patientId:pid(),familyVisibility:mentalVisible.value?'VISIBLE':'PRIVATE'})},'量表推送计划已设置')}
...
async function loadTab(){try{if(!clinicians.value.length)clinicians.value=(await api.listClinicians()).data||[];const loaders={measurements:loadMeasurements,appointments:loadAppointments,recovery:loadRecovery,specialty:loadSpecialty,emergency:loadEmergencyCard,mental:loadMental,privacy:loadGrants,operations:loadOperations};if(loaders[tab.value])await loaders[tab.value]()}catch{}}
...
watch(currentPatientId,()=>{
  ...
  measurements.value=[];appointments.value=[];visits.value=[];prescriptions.value=[];Object.keys(specialtyRows).forEach(key=>{specialtyRows[key]=[]});plans.value=[];rehabRows.value=[];emergencyCard.value=null;mentalRows.value=[];grants.value=[];doctorSchedules.value=[]
