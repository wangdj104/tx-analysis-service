...
CREATE TABLE IF NOT EXISTS `platform_branding` (`id` TINYINT NOT NULL,`platform_name` VARCHAR(80) NOT NULL,`organization_name` VARCHAR(120) NOT NULL DEFAULT '',`logo_data` LONGTEXT NOT NULL,`page_background` CHAR(7) NOT NULL DEFAULT '#f5f7fb',`ownership_text` VARCHAR(240) NOT NULL,`updated_by` BIGINT DEFAULT NULL,`updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='平台品牌配置';
INSERT INTO `platform_branding` (`id`,`platform_name`,`organization_name`,`logo_data`,`page_background`,`ownership_text`) VALUES (1,'澄心健康','澄心健康','/logo.svg','#f5f7fb','© 2026 澄心健康 版权所有') ON DUPLICATE KEY UPDATE id=VALUES(id);
INSERT INTO `sys_menu` (`id`,`parent_id`,`menu_name`,`menu_code`,`menu_path`,`menu_icon`,`permission`,`menu_type`,`sort_order`,`status`) VALUES (41,5,'平台品牌','platform-branding','/system/branding','Brush','branding:manage',1,5,1) ON DUPLICATE KEY UPDATE parent_id=VALUES(parent_id),menu_name=VALUES(menu_name),menu_path=VALUES(menu_path),menu_icon=VALUES(menu_icon),permission=VALUES(permission),sort_order=VALUES(sort_order),status=VALUES(status);
INSERT INTO `sys_role_menu` (`role_id`,`menu_id`) SELECT r.id,41 FROM sys_role r WHERE r.role_code='admin' ON DUPLICATE KEY UPDATE role_id=VALUES(role_id);
