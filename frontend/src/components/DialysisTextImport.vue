<template>
  ...
  <el-dialog v-model="visible" title="通过模板导入透析记录" width="min(960px,96vw)">
    <p>每次透析请以日期开始，体重单位为 kg。超滤量、间隔天数和上次透后体重可留空，系统会根据已有记录推算。</p>
    <el-button @click="insertTemplate">插入模板</el-button><el-button @click="copyTemplate">复制模板</el-button>
    <el-input v-model="text" type="textarea" :rows="14" placeholder="请在此粘贴填写完成的模板" style="margin:14px 0"/>
    ...
    <template #footer><el-button @click="visible=false">关闭</el-button><el-button type="success" :loading="busy" :disabled="!result||result.errors.length>0||!result.rows.length" @click="confirm">导入至当前家庭成员</el-button></template>
  </el-dialog>
</template>
<script setup>
...
async function preview(){busy.value=true;const version=generation;try{const r=await request.post('/dialysis/text-import/preview',{patientId:props.patientId,text:text.value});if(version===generation)result.value=r.data}finally{busy.value=false}}
async function confirm(){if(busy.value)return;busy.value=true;try{const r=await request.post('/dialysis/text-import/confirm',{patientId:props.patientId,text:text.value});ElMessage.success(`已导入 ${r.data.added} 条记录，跳过 ${r.data.skipped} 个已有日期。`);visible.value=false;result.value=null;emit('saved')}finally{busy.value=false}}
