<template>
  ...
  <el-dialog v-model="visible" title="Import dialysis records from a template" width="min(960px,96vw)">
    ...
    <el-button @click="insertTemplate">Insert template</el-button><el-button @click="copyTemplate">Copy template</el-button>
    <el-input v-model="text" type="textarea" :rows="14" placeholder="Paste the completed template here" style="margin:14px 0"/>
    ...
    <template #footer><el-button @click="visible=false">Close</el-button><el-button type="success" :loading="busy" :disabled="!result||result.errors.length>0||!result.rows.length" @click="confirm">Import for selected family member</el-button></template>
  </el-dialog>
</template>
<script setup>
...
async function confirm(){if(busy.value)return;busy.value=true;try{const r=await request.post('/dialysis/text-import/confirm',{patientId:props.patientId,text:text.value});ElMessage.success(`Imported ${r.data.added} records; skipped ${r.data.skipped} existing dates.`);visible.value=false;result.value=null;emit('saved')}finally{busy.value=false}}
