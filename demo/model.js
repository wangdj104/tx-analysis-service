...
(() => {
  ...
  function currentConsultation(state) { return state.consultations.find(item => item.patientId === Number(state.patientId)) || state.consultations[0]; }
  function sendChatMessage(state, text, role = state.role, now = new Date()) { const clean=String(text||'').trim(); if(!clean)throw new Error('empty-message'); const names={doctor:bi('Dr. Sarah Chen','陈医生'),patient:current(state).name,family:bi('Wei Zhang','张伟'),admin:bi('Platform administrator','平台管理员')}; const chat=currentConsultation(state); chat.messages.push({id:Date.now(),sender:role,name:names[role]||names.patient,text:bi(clean,clean),time:clock(now)}); state.audit.unshift({time:clock(now),actor:names[role]||names.patient,action:bi('Sent consultation message','发送问诊消息'),result:'success'}); return chat.messages.at(-1); }
  function addDemoReply(state, language = 'en', now = new Date()) { const chat=currentConsultation(state), text=language==='zh'?'已收到这条演示消息。请继续观察症状；如明显加重，请立即联系急救服务。':'I received this demo message. Keep observing the symptom; if it becomes severe, contact emergency services immediately.'; chat.messages.push({id:Date.now()+1,sender:'doctor',name:bi('Dr. Sarah Chen','陈医生'),text:bi(text,text),time:clock(now)}); return chat.messages.at(-1); }
