...
-- MySQL 5.7-compatible conditional upgrades for reviewable legacy records.
...
SELECT 0,'Doctor Workspace','doctor-workspace','/doctor-workspace','FirstAidKit','doctor-workspace:view',1,1,1 FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `sys_menu` WHERE `menu_code`='doctor-workspace');
UPDATE `sys_menu` SET `menu_name`='Doctor Workspace',`menu_path`='/doctor-workspace',`menu_icon`='FirstAidKit',`permission`='doctor-workspace:view',`status`=1 WHERE `menu_code`='doctor-workspace';
